...
let vehicles=[],selected=null,target=null,qrScanner=null,toastTimer,detailRequest=0;
...
async function openVehicle(id){const requestId=++detailRequest;try{const history=await repo.history(id);if(requestId!==detailRequest)return;const found=vehicles.find(v=>v.id===id);if(found)found.history=history;}catch(e){toast(e.message);return;}selected=vehicles.find(v=>v.id===id);if(!selected){toast('Fahrzeug nicht gefunden.');return;}target=selected.areaId;const v=selected;$('detail-content').innerHTML=`<h2 id="detail-title" class="detail-name">${escape(v.brand)} ${escape(v.model)}</h2><p class="muted">${escape(v.condition)} · Testfahrzeug</p><p class="detail-vin">FIN ${escape(v.vin)}</p><div class="detail-summary"><span>Zuletzt gespeicherter Standort</span><strong>${areaLabel(v.areaId)}</strong><small>${date(v.updatedAt)}</small></div><label>Neues Parkareal auswählen</label><div class="area-grid" role="group" aria-label="Neues Parkareal">${AREAS.map(a=>`<button data-area="${a.id}" aria-label="${a.name}" aria-pressed="${a.id===target}">${String(a.id).padStart(2,'0')}</button>`).join('')}</div><p id="move-error" class="error" role="alert"></p><button id="save" class="primary full" ${target===v.areaId?'disabled':''}>Standort speichern</button><div class="history"><h3>Standortverlauf</h3><p>Die letzten 100 Änderungen</p>${v.history.length?v.history.map(h=>`<div class="history-item">${areaLabel(h.from)} → ${areaLabel(h.to)}<small>${date(h.at)} · Gespeichert in Supabase</small></div>`).join(''):'<p>Noch keine Standortänderungen.</p>'}<p>QR-ID: <span class="detail-vin">${escape(v.qrId)}</span></p></div>`;if(!$('detail').open)$('detail').showModal();$('save').onclick=saveMove;}
...
async function stopCamera(){try{if(qrScanner){const state=qrScanner.getState?.();if(state===2||state===3)await qrScanner.stop();await qrScanner.clear();}}catch{}qrScanner=null;$('qr-reader').hidden=true;$('camera').disabled=false;$('camera').textContent='Kamera starten';}
...
function showQR(value){const v=resolveQR(value);if(!v){$('scan-error').textContent='Dieser Code gehört zu keinem der freigeschalteten Testfahrzeuge. Zum Testen: VC-DEMO-0001.';return false;}$('scanner').close();openVehicle(v.id);return true;}
$('qr-form').onsubmit=e=>{e.preventDefault();showQR($('qr-input').value);};
$('camera').onclick=async()=>{try{if(!window.Html5Qrcode)throw new Error('QR-Scanner konnte nicht geladen werden. Bitte Seite neu laden.');$('scan-error').textContent='';$('camera').disabled=true;$('camera').textContent='Kamera wird gestartet …';$('qr-reader').hidden=false;qrScanner=new Html5Qrcode('qr-reader');await qrScanner.start({facingMode:'environment'},{fps:10,qrbox:{width:240,height:240},aspectRatio:1},async decodedText=>{await stopCamera();showQR(decodedText);},()=>{});$('camera').textContent='Kamera läuft';$('scan-message').textContent='QR-Code ruhig in den Rahmen halten.';}catch(e){await stopCamera();const msg=String(e?.message||e);$('scan-error').textContent=/permission|notallowed|denied/i.test(msg)?'Kamerazugriff nicht erlaubt. Bitte Kamera für diese Website in Safari/Chrome freigeben.':'Kamera konnte nicht gestartet werden: '+msg;}};
